...
/*
 ******************************************************************************
 * XRT based test wrapper for sequence alignment
 * 2 inputs and 1 output
 * However when running AIE the inputs (reference and query) should be part of the call
 ******************************************************************************
 */
template <typename T1, typename T2, void (*init_bufRef)(T1 *, int),
          void (*init_bufQry)(T1 *, int), void (*init_bufDP)(T2 *, int),
          int (*verify_results)(T1 *, uint32_t, T1 *, uint32_t, T2 *, int, int)>
int setup_and_align_aie(uint32_t REF_VOLUME, uint32_t QRY_VOLUME, uint32_t DP_VOLUME,
  ...
  auto bo_in1 = xrt::bo(device, REF_VOLUME * sizeof(T1), XRT_BO_FLAGS_HOST_ONLY,
                        kernel.group_id(3));
  auto bo_in2 = xrt::bo(device, QRY_VOLUME * sizeof(T1), XRT_BO_FLAGS_HOST_ONLY,
                        kernel.group_id(4));
  auto bo_out = xrt::bo(device, DP_VOLUME * sizeof(T2), XRT_BO_FLAGS_HOST_ONLY,
                        kernel.group_id(5));
  ...
  T2 *bufOut = bo_out.map<T2 *>();
  char *bufTrace = bo_trace.map<char *>();
  uint32_t *bufCtrlPkts = bo_ctrlpkts.map<uint32_t *>();

  init_bufRef(bufIn1, REF_VOLUME);
  init_bufQry(bufIn2, QRY_VOLUME);
  init_bufDP(bufOut, DP_VOLUME); // <<< what size do I pass it?
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    if (myargs.do_verify) {
      ...
      errors +=
          verify_results(bufIn1, REF_VOLUME, bufIn2, QRY_VOLUME, bufOut, DP_VOLUME, myargs.verbosity);
  ...
  if (myargs.verbosity >= 1) {
    std::cout << "Avg NPU time: " << npu_time_total / myargs.n_iterations << "us." << std::endl;
    if (macs > 0)
  ...
  if (!errors) {
    if(myargs.verbosity == 0) std::cout << "PASS!" << std::endl; // Cleaner for multiple tests
    else std::cout << "\nPASS!\n\n";
    return 0;
  } else {
    if(myargs.verbosity == 0) std::cout << "Error count: " << errors << std::endl; // Cleaner for multiple tests
    else std::cout << "\nFailed.\n\n";
    return 1;
  }
}